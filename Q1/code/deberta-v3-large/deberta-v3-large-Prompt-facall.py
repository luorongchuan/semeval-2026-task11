...
class TargetedFocalLoss(nn.Module):
    def __init__(self, gamma: float = 2.0, alpha: float = 1.0):
        super().__init__()
        self.gamma = gamma
        self.alpha = alpha

    def forward(self, logits, labels, plausibilities):
        """
        logits: [B, 2]
        labels: [B] (0/1)
        plausibilities: [B] (0/1, -1 for missing)
        """
        # 标准交叉熵（未 reduce）
        ce_loss = F.cross_entropy(logits, labels, reduction='none')  # [B]

        # 计算 pt = exp(-ce_loss) → 预测概率的置信度
        pt = torch.exp(-ce_loss)

        # 构建 mask：只对 plausible-invalid 样本 (plaus=1, label=0) 加 focal 权重
        mask = (plausibilities == 1) & (labels == 0)  # [B], bool

        # 默认权重为 1（标准 CE）
        weights = torch.ones_like(ce_loss)

        # 对目标样本应用 focal 权重
        weights[mask] = self.alpha * (1 - pt[mask]) ** self.gamma

        # 加权 loss
        loss = (weights * ce_loss).mean()
        return loss

class TargetedFocalTrainer(Trainer):
    def __init__(self, *args, focal_gamma=2.0, focal_alpha=1.0, **kwargs):
        super().__init__(*args, **kwargs)
        self.loss_fn = TargetedFocalLoss(gamma=focal_gamma, alpha=focal_alpha)

    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        # print("Keys in inputs:", list(inputs.keys()))  # 👈 加这一行
        labels = inputs.pop("labels")
        plaus = inputs.pop("plausibility", None)  # ← 关键：需要 plausibility 作为输入！
        # print("Plausibility sample:", plaus[:5] if plaus is not None else "None")  # 👈 再加一行

        outputs = model(**inputs)
        logits = outputs.logits

        if plaus is not None:
            loss = self.loss_fn(logits, labels, plaus)
        else:
            # 如果没有 plausibility（如测试时），回退到 CE
...
def train_model(data, cfg: NLIConfig):
    ...
    def preprocess_function(examples, tokenizer):
        ...
        # 👇 关键：保留 labels 和 plausibility
        encoding["labels"] = examples["labels"]
        encoding["plausibility"] = examples["plausibility"]  # ← 新增这一行！
    ...
    # print("\nPlausibility value counts:")
    # print(df_train['plausibility'].value_counts(dropna=False))
    ...
    training_args = TrainingArguments(
        ...
        report_to=["none"],
        remove_unused_columns=False,  # ←←← 必须加这一行！
    )

    trainer = TargetedFocalTrainer(
        model=model,
        args=training_args,
        ...
        focal_alpha=0.5,      # 可调：类别不平衡时设 alpha<1 for majority class
        focal_gamma=0.5       # 推荐值：1~5，越大越聚焦难样本
    )
