...
class SyllogismProcessor:
    ...
    def _initialize_constants(self) -> None:
        ...
        self.unplausibility_syllogisms = {
                    "AA3" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "AI1" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "AI3" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "AO1" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "AO2" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "IA2" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "IA3" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "II1" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "II2" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "II3" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "II4" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "IO1" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "IO2" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "IO3" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "IO4" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "OA1" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "OA2" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "OI1" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "OI2" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "OI3" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "OI4" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "OO1" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "OO2" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "OO3" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "OO4" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                    "OE1" : ["All A are C", "some A are C", "some C are A","All C are A"],
                    "OE2" : ["All A are C", "some A are C", "some C are A","All C are A"],
                    "OE3" : ["All A are C", "some A are C", "some C are A","All C are A"],
                    "OE4" : ["All A are C", "some A are C", "some C are A","All C are A"],
                    "EO1" : ["All A are C", "some A are C", "some C are A","All C are A"],
                    "EO2" : ["All A are C", "some A are C", "some C are A","All C are A"],
                    "EO3" : ["All A are C", "some A are C", "some C are A","All C are A"],
                    "EO4" : ["All A are C", "some A are C", "some C are A","All C are A"],
                    "EE1" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "EE2" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "EE3" : ["All C are A",  "Some A are not C","No A are C","No C are A"],
                    "EE4" : ["All A are C",  "Some C are not A","No A are C","No C are A"],
                }
...
class SyllogismDataset(SyllogismProcessor):
    ...
    def _load_vocabulary(self) -> None:
        ...
        f"syllowords_{self.config.test_data_type}_all50.json"
    )
    ...
    def _process_structure(self, structure: str, problem_type: str) -> List[tuple]:
        # """仅处理结论为 'Nothing follows' 的结构，并从 self.plausibility_syllogisms[problem_type] 中遍历生成无效且不合理的样本"""
        results = []

        # 提取结论部分
        conclusion_part = structure.split("<CONCLUSION> ")[1].strip()
        conclusions = [c.strip().lower() for c in conclusion_part.replace("|", "or").split(" or ")]

        # 只保留 "Nothing follows" 的结构
        if "nothing follows" not in conclusions:
            return results

        # 获取替换词（A/B/C 的具体名词）
        if problem_type not in self.syllowords:
            return results
        to_substitute = self.syllowords[problem_type]

        # 获取该 problem_type 对应的不合理结论列表
        if problem_type not in self.unplausibility_syllogisms:
            return results  # 如果没有定义，跳过
        candidate_conclusions = self.unplausibility_syllogisms[problem_type]

        # 提取前提模板
        premise_text = structure.split("\n<STRUCTURE> ")[1].split("\n<CONCLUSION>")[0].replace(" / ", ".\n") + "."
        raw_template = f"Syllogism:{premise_text} "

        # 遍历每组替换词（每组是一个 [noun_a, noun_b, noun_c]）
        for nouns in to_substitute:
            noun_a, noun_b, noun_c = nouns
            ...
            # 遍历该 problem_type 下所有预设的合理结论
            for c in candidate_conclusions:
                # 清理并标准化结论（去掉末尾句号，统一加）
                clean_conclusion = c.strip().rstrip('.')
                ans_replaced = clean_conclusion + "."
                ...
                # 标记：无效 + 合理（按你要求）
                is_valid = False
                plausibility =  False  # 明确设为不合理

                results.append((raw_template, txt, ans_replaced, clean_conclusion, is_valid, plausibility))
    ...
    def substitute_words(self, df: pd.DataFrame, split: str = "train"):
        ...
        believable_vocab_path = os.path.join(self.config.vocab_dir, "syllowords_believable.json")
        with open(believable_vocab_path) as f:
            believable_vocab = json.load(f)  # dict like {"AA1": [["dog", "animal", "mammal"], ...], ...}
        substituted_data = []
        ...
        for _, row in df.iterrows():
            ...
            original_ans_clean = answer_full.strip().rstrip(".")
            # --- Case 2: Replace ONLY premise (set validity=False, plausibility unchanged) ---
            
            modified_premise = replace_tokens(premise_part, token_map)
            # substituted_data.append((modified_premise, answers, False, plausibility))

            # # --- Case 4: Replace ONLY conclusion (set plausibility=False, validity unchanged) ---
            
            # modified_conclusion = replace_tokens(original_ans_clean + ".", token_map)
            # substituted_data.append((original_text, modified_conclusion, False, False))  # validity unchanged

            # --- Case 3: Replace BOTH premise and conclusion (validity unchanged, plausibility=False) ---
            # if ( random.random() < 0.5):  # ← 关键：50% 替换概率
            modified_conclusion = replace_tokens(original_ans_clean + ".", token_map)
            modified_premise = replace_tokens(premise_part, token_map)
            # substituted_data.append((modified_premise, modified_conclusion, validity, False))
           # --- Case 5: Replace ONLY conclusion with PLAUSIBLE WORDS (validity=unchanged, plausibility=True) ---
            # pt = problem_type
            # if pt in believable_vocab and believable_vocab[pt]:
            #     # Randomly pick one noun triplet for this syllogism type
            #     noun_a, noun_b, noun_c = random.choice(believable_vocab[pt])
                
            #     # Build mapping for A/B/C
            #     clean_conclusion = answer_full.strip().rstrip(".")
            #     plausible_conclusion = clean_conclusion + "."
                
            #     # Perform safe replacement (preserve word boundaries)
            #     for placeholder, real_word in [("A", noun_a), ("B", noun_b), ("C", noun_c)]:
            #         plausible_conclusion = plausible_conclusion.replace(f"{placeholder} ", f"{real_word} ")
            #         plausible_conclusion = plausible_conclusion.replace(f"{placeholder}.", f"{real_word}.")
                
            #     # Keep original premise text, only change conclusion
            # substituted_data.append((original_text, plausible_conclusion, False, True))
...
if __name__ == "__main__":
    ...
    output_file = "/home/luorongchuan/workspace_134/Semeval2026/Augmentation_data/soft-syllogistic-reasoners-main/scripts/dataset3/unvalidity_unplausibility.json"  # 可以指定完整路径
    # 将生成的数据存储为 JSON 格式
