...
# 如果你仍想用 hf-mirror（其实不需要了），可注释掉上面两行
...
class QwenForSequenceClassification(nn.Module):
    def __init__(self, model_name_or_path: str, num_labels: int = 2, dropout: float = 0.1):
        ...
        # 从本地路径加载 Qwen2Model
        self.qwen = Qwen2Model.from_pretrained(
            model_name_or_path,
...
class NLIConfig:
    # ✅ 改为本地路径
    local_model_path: str = "/home/luorongchuan/workspace_134/Semeval2026/Q3/local-model/Qwen2.5-1.5B-Instruct"
    max_length: int = 512
...
# -----------------------------
# 数据集处理、加载、内容效应等函数（保持不变）
# -----------------------------

# （以下函数与你原代码完全一致，仅省略以节省篇幅，实际使用时保留）
...
def train_model(data, cfg: NLIConfig):
    ...
    training_args = TrainingArguments(
        output_dir='/home/luorongchuan/workspace_134/Semeval2026/Q3/local-code-noaug/qwen2.5-1.5b-noaug/out',
        eval_strategy="epoch",
    ...
    model_save_path = "/home/luorongchuan/workspace_134/Semeval2026/Q3/local-code-noaug/qwen2.5-1.5b-noaug/out"
    os.makedirs(model_save_path, exist_ok=True)

    # 替换原来的保存代码
    model.save_pretrained(model_save_path)  # ← 这会保存整个模型（含 resize 后的 embedding）
...
def predict_on_test_set(test_filepath: str, model_path: str, tokenizer_path: str, output_json_path: str, max_length: int = 512):
    ...
    # ✅ 加载自定义模型结构 + 本地权重
    model = QwenForSequenceClassification(
        model_name_or_path=tokenizer_path,  # 用于初始化 config 和 backbone
        num_labels=2
    )
...
if __name__ == "__main__":
    ...
    cfg = NLIConfig(
        local_model_path="/home/luorongchuan/workspace_134/Semeval2026/Q3/local-model/Qwen2.5-1.5B-Instruct",
        max_length=512,
        batch_size=16,
        epochs=5,
        learning_rate=1e-5,
    ...
    OUTPUT_FILE = "/home/luorongchuan/workspace_134/Semeval2026/Q3/local-code-noaug/qwen2.5-1.5b-noaug/predictions.json"
    MODEL_SAVE_DIR = "/home/luorongchuan/workspace_134/Semeval2026/Q3/local-code-noaug/qwen2.5-1.5b-noaug/out"
