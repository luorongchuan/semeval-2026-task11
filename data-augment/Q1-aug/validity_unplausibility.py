...
class SyllogismDataset(SyllogismProcessor):
    ...
    # """Substitute words in syllogism data with gibberish: 80% keep original, 20% replace both premise and conclusion."""
        ...
        for _, row in df.iterrows():
            ...
            answer_full = row["ans_type"]

            if random.random() < 0.5:
                # 50%: keep original
                substituted_data.append((original_text, answers, validity, plausibility))
            else:
                # 50%: replace both premise and conclusion with gibberish
...
if __name__ == "__main__":
    ...
    output_file = "/home/luorongchuan/workspace_134/Semeval2026/Augmentation_data/soft-syllogistic-reasoners-main/scripts/dataset3/validity_unplausibility.json"  # 可以指定完整路径
    # 将生成的数据存储为 JSON 格式
